...
#include "UnityCG.cginc"
	...
	bool isBlack(float3 pos) {
		...
		float data = tex3D(Legend_Data, pos_righthanded).r; //just r value is fine, since we're dealing with black&white so its either 0 or 1
		...
		if (data == 0) {		//if it's black we want to return true
			return true;
		}
		else {
			return false;
